...
% Parámetros por defecto para cancion
\pgfkeys{
    /cancion/.is family, /cancion,
    titulo/.initial=,
    autor/.initial=,
    cejilla/.initial=,
}

\NewDocumentEnvironment{cancion}{ O{} }{%
    % Procesar parámetros
    \pgfkeys{/cancion, #1}%
    
    % Definir variables
    \def\tempTitulo{\pgfkeysvalueof{/cancion/titulo}}%
    \def\tempAutor{\pgfkeysvalueof{/cancion/autor}}%
    \def\tempCejilla{\pgfkeysvalueof{/cancion/cejilla}}%

    % Añadir al índice
    \stepcounter{contador_canciones}%
    \phantomsection%
    \ifthenelse{\isempty{\tempAutor}}%
        {\addcontentsline{toc}{subsection}{%
            \hbox to \leftmargin{\hss\arabic{contador_canciones}.}{ \tempTitulo}\ %
            }} % Si no tiene autor especificado
        {\addcontentsline{toc}{subsection}{% Si tiene autor especificado    
            \hbox to \leftmargin{\hss\arabic{contador_canciones}.} \tempTitulo\ (\textit{\tempAutor})%
        }} % 
   
    % 4. DISEÑO VISUAL
    \noindent%
    % Número
    \makebox[0.8cm][l]{\large\textbf{\arabic{contador_canciones}.}}%
    %
    % Título
    \parbox[t]{\dimexpr\linewidth-0.8cm\relax}{%
        \large\textbf{\tempTitulo}%
    }\\%
    %
    % Autor
    \ifx\tempAutor\empty%
        % Cejilla
        \ifx\tempCejilla\empty\else%
            \hfill\textit{\footnotesize cej. t.\tempCejilla}%
        \fi%
    \else%
        \noindent\hspace*{0.8cm}\small\textit{\tempAutor}% 
        % Cejilla
        \ifx\tempCejilla\empty\else%
            \hfill\textit{\footnotesize cej. t.\tempCejilla}%
        \fi%
    ...
    \begin{spacing}{\interlineadocanciones}%
        \obeyspaces\spaceskip=0.15cm\relax%
}{%
    \end{spacing}%
}
